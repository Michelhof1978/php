...
<?php 
    ...
    function checkTableau($tableau){// La fonction va permettre de renvoyer true si le tableau n'a que des valeurs paires et false s'il n'a pas que des valeurs paire
        for($i = 0; $i <= count($tableau)-1 ; $i++){//on commence à l'indice  0 du tableau et non 1, on va ainsi parcourir le tableau grâce à count et on s'arrêtera bien à -1 comme on démarre à 0
            if($tableau[$i] % 2 !== 0){//Pour savoir si c'est paire, on utilisera le modulo, ici, on vérifie si c'est divisible par 2 et si c'est le cas, le resultat du modulo est égale à 
                return false;
            }
        }
        return true;
